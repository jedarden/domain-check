// Package httpclient provides SSRF-safe HTTP client utilities for querying
// RDAP registry servers. It includes private IP blocking, URL allowlists,
// redirect validation, and per-registry connection pool tuning.
package httpclient
...
// Values are aligned with the concurrency limits in ratelimit package so the pool
...
// NewTransportForPool creates an http.Transport with pool sizing for a specific registry.
...
// sized to the known registry concurrency limits defined in RegistryPoolConfigs.
