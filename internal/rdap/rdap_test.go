...
// mockAllowList is a test implementation of AllowList.
type mockAllowList struct {
	allowed map[string]bool
}

func newMockAllowList(urls []string) *mockAllowList {
	al := &mockAllowList{allowed: make(map[string]bool)}
	for _, u := range urls {
		if host := extractHost(u); host != "" {
			al.allowed[strings.ToLower(host)] = true
		}
	}
	return al
}

func (m *mockAllowList) Allowed(url string) bool {
	host := extractHost(url)
	if host == "" {
		return false
	}
	return m.allowed[strings.ToLower(host)]
}
...
func TestRDAPFixtureParsing(t *testing.T) {
	...
	for _, tt := range registeredTests {
		t.Run(tt.fixture, func(t *testing.T) {
			data, err := os.ReadFile(filepath.Join("..", "checker", "testdata", "rdap", tt.fixture))
			if err != nil {
...
func TestRDAPAvailableFixtures(t *testing.T) {
	...
	for _, fixture := range fixtures {
		t.Run(fixture, func(t *testing.T) {
			data, err := os.ReadFile(filepath.Join("..", "checker", "testdata", "rdap", fixture))
			if err != nil {
				t.Fatalf("failed to read fixture: %v", err)
			}
...
func newTestRDAPClient(server *httptest.Server) *RDAPClient {
	...
	allowlist := newMockAllowList([]string{server.URL})
...
func FuzzParseRDAPResponse(f *testing.F) {
	...
	fixtureDir := filepath.Join("..", "checker", "testdata", "rdap")
	fixtures := []string{
