...
| 2026-09-09 | §8 battery of the bf-173o7e remediation proposal re-run by domchk-fc2d8600 (implement leg of the bf-173o7e chain; verification-only disposition per the proposal's §7 and the domchk-5355a695 precedent) at HEAD `d422c29` (dispatch base; this row's commit ships on top) — the proposal's seven-layer confirmation, load 3.2 | **all green, first pass**: check-repo-health exit 0; gc-config `--verify` exit 0 (effective worst case ≈3072 MiB within the 12GiB-scope ceiling); `safe-git-gc.sh --check-only` preflight passes (rc 1 = "GC not needed" verdict); hooks `--check` rc 0; 0 tracked `.beads` files; `auto-gc-trigger --dry-run` "GC not needed"; backlog CLEAR (0 unpushed); preflight all-pass; event-mode `clear`; breaker `{}`; 8/8 timers future-triggered. Death-op replay 17/17 — bounded push peak RSS 232,504 KB, bare `git gc --aggressive --prune=now` exited 0 under MemoryMax=768M with pack-objects peak RSS 320,440 KB (the Aug-14 crash run exceeded the 12 GiB scope). `test-safe-git-gc-limits` 33/33. Live state: repo 105 MB / 1 pack (12,607 objects / 100.70 MiB) / 47 loose / 316 KiB / 0 garbage / 0 unpushed. Zero functional changes — the remediation is the seven landed layers; this leg's only output is this row. |
| 2026-09-09 | The memcg-OOM mechanism battery re-run by domchk-5e9399b3 (implement leg of the bf-4hp9p chain — the five 2026-08-16 `CONSTRAINT_MEMCG` git kills of bf-4hp9p's processors inside the 12 GiB dispatch scope, kernel-proven per the analyze leg `docs/crash-investigations/bf-4hp9p-analyze-domchk-582a1137-2026-09-09.md`; verification-only disposition per that RCA's §7 "no new work proposed" and the domchk-fc2d8600 precedent) at HEAD `f879fa6` (dispatch base; this row's commit ships on top) — the seven layers named by the RCA's §7, load 3.8 rising to 6.2 under the batch | **all green, first pass**: check-repo-health exit 0 (0 unpushed backlog); gc-config `--verify` exit 0 (effective worst case ≈3072 MiB, windowMemory/deltaCache/threads all supplied repo-locally, within the 12GiB-scope ceiling); `safe-git-gc.sh --check-only` rc 1 = "GC not needed" verdict; hooks `--check` rc 0; 0 tracked `.beads` files (`.gitignore` still carries `.beads/`, `*.db`, `*.jsonl`); `auto-gc-trigger --dry-run` "GC not needed"; preflight exit 0 (0 failed); event-mode `clear` (PSI some avg60 0.00%); breaker `{}`; 8/8 timers future-triggered. Death-op replay **17/17** — bounded push peak RSS 232,280 KB and bare `git gc --aggressive --prune=now` exited 0 under MemoryMax=768M with pack-objects peak RSS **320,492 KB**, both < the 700 MiB assertion cap (~40× under the 12 GiB scope whose breach SIGKILLed the five bf-4hp9p processors; the era's unbounded runs exceeded 12 GiB). `test-safe-git-gc-limits` **33/33**; `test-crash-alert-fixes` all pass; `test-closed-bead-filter` all pass (repo cwd) — the alert-layer gates the RCA credits with ending this chain's crash→alert→investigator-killed loop. Live state: `.git` 105 MB / 1 pack 100.70 MiB / 66 loose / 444 KiB / 0 garbage / 0 unpushed. Zero functional changes — the remediation is the seven landed layers; this leg's only output is this row. Attribution note: `go build ./...` in the shared worktree fails only inside a co-tenant's uncommitted `internal/watch/manager.go` edit (110 insertions; `.domain.Parse` absent at HEAD); a pure HEAD extract fails with `undefined: NewResourceMonitor` (the known untracked `resource_monitor*.go` gap since e4fcbec) — neither touches this docs-only leg. |

| 2026-09-09 | Verify leg of the bf-4hp9p chain (domchk-24615bf5 — "Verify crash fix and confirm resolution", the fourth leg after gather domchk-fb213816 / analyze domchk-582a1137 / implement domchk-5e9399b3) at HEAD `70e8f1f` (dispatch base; this row's commit ships on top) — the task's six acceptance criteria mapped to live checks, load 4.0–5.8 | **all green, first pass — fix verified, no recurrence**: (1) *Agent re-run on the crash work* — this dispatch IS an agent re-run on the bf-4hp9p crash chain and completes exit 0 (no prior attempt landed: no `--grep domchk-24615bf5` commit, no work-completion record, Notes empty); the original work itself had already completed without loss — the analyze leg's terminal processor claim exited 0 at 2026-08-16T14:52:48.878Z, the deliverable (e210c84) landed 26 s before kill #4, and the target alert bead bf-4hp9p is **Closed rev 21** (2026-08-25, re-read live), so the "mark the original crash bead resolved" criterion is already satisfied and nothing was reopened. (2) *Agent completes without crash* — fleet census over 3 days of worker logs (693,516 lines): **798 `outcome=Success` / `exit_code=0`** vs 327 `outcome=Failure` (`exit_code=1`, the known service-class wave — a different failure class, no alert mints) and 7 `exit_code=124` timeouts, **0 Crash / 0 exit −1**, 4,281 completions all `was_interrupted=false`. (3) *Sustained load test (the crash was resource-related)* — `test-gc-memory-bounds.sh` **17/17**: it drives the era's death operations through a 768 MiB cgroup — bounded push peak RSS **232,416 KB**, bare `git gc --aggressive --prune=now` exit 0 with pack-objects peak RSS **320,516 KB** (~40× under the dispatch scope, re-read live this dispatch at 12,884,901,888 B = 12 GiB); `test-safe-git-gc-limits.sh` **33/33**. (4) *Recurrence monitoring* — kernel census since the Aug-17 remediation era: 133 `CONSTRAINT_MEMCG` kills, **0 inside needle dispatch scopes**; 100% attributed to named synthetic scopes (`safe-git-gc-run-*` = the limits suite's deliberate small-cap negative test; `gcmb-*`/`bf1s6c3-*`/`bf4yjq-*` = death-op replay scopes; `mw-*`/`probe-hog-*`/`run-isolated-*` = investigation-era diagnostics), clustered exactly on battery-run days (Sep 02: 15, Sep 06: 33, Sep 07: 63, Sep 08: 15, Sep 09: 7). One Sep-07 09:54 EDT `gcmb-bare-aggressive` replay exceeded its own 768 MiB **test** cap (git anon-rss 783,628 KB — the cap caught it inside a synthetic scope; no dispatch scope involved; every recorded run since is 17/17, including this one). `crash-pattern-detection.sh` (24 h): "No crashes detected … System Status: STABLE". **0 ALERT beads minted since bf-4hp9p closed (2026-08-25)** out of 1,584 ever — the crash→alert→investigator loop this repo's fixes target has gone quiet. (5) *Documentation* — this row. Attribution note: the co-tenant worktree carries uncommitted edits to `crash-alert-manager.sh` and `crash-pattern-detection.sh`, so the alert-layer suites were re-run from a `git archive HEAD` extract and pass there too (`test-crash-alert-fixes` rc 0; `test-closed-bead-filter` rc 0 from the repo cwd, both ways); the worktree pattern-detector differs from HEAD only by an additive co-tenant coverage extension (domchk-4b0334eb), so the 24 h STABLE verdict ran a superset of HEAD's checks. Live gates: check-repo-health exit 0 (0 unpushed); gc-config `--verify` exit 0 (effective worst case ≈3072 MiB within the 12 GiB scope); `safe-git-gc.sh --check-only` rc 1 = "GC not needed" verdict; hooks `--check` rc 0; 0 tracked `.beads` files; auto-gc dry-run "GC not needed"; preflight exit 0; event-mode `clear`; breaker `{}`; 8/8 timers future-triggered. Live state: `.git` 105 MB / 73 loose objects / 492 KiB / 0 garbage / 0 unpushed. Sibling note: alert bead domchk-6ce337d0 (rev 16, one of the five kill-wave alerts, blocked by this leg) already has its verification report committed (32a4b7b) and belongs to its own close dispatch — not touched here. |
| 2026-09-09 | Full battery re-run by domchk-1a78af71 (implement-and-verify leg of the bf-4yjq chain — the 2026-08-12 push-side storm, 50 × `exit −1` at `git push` over the 18 GB store; verification-only disposition per the tasked report `docs/crashes/bf-4yjq/report.md` §9 "new recommendation: none" and the domchk-fc2d8600 / domchk-5e9399b3 precedent; the target bead bf-4yjq is already **Closed rev 2** 2026-08-17T00:14:14Z with a verification-bearing close reason, so the task's "close bf-4yjq as resolved: mitigated" criterion is satisfied by prior state) at HEAD `edee672` (dispatch base; this row's commit ships on top) — 13 live checks + 19 tracked suites, sequential batches at load 3.7–5.1 | **all green, first pass**: check-repo-health exit 0; gc-config `--verify` exit 0 (effective worst case ≈3072 MiB, windowMemory/deltaCache/threads all supplied repo-locally, within the ceiling); hooks `--check` rc 0 ("byte-identical to tracked source"); 0 tracked `.beads` files (`.gitignore` still carries `.beads/`, `*.db`, `*.jsonl`); `auto-gc-trigger --dry-run` "GC not needed"; backlog CLEAR (0 unpushed); service-monitor "PRE-FLIGHT CHECK PASSED"; resource-monitor pressure 0% / UNSAFE_GC none; preflight **5/5**; event-mode `clear` (PSI some avg60 0.00%); breaker `{}`; **8/8** timers future-triggered; crash-pattern detector **STABLE — 0 new crashes in the 24 h horizon**, the 50 bf-4yjq/bf-1s6c3 kills on file as pre-horizon history. Death-op replay **17/17** — this bead's chain is the *push-side* storm, and the replay's bounded push peaked at **232,480 KB** with pack-objects **320,536 KB**, both < the 700 MiB assertion cap and ~40× under the dispatch scope, **re-read live this leg at memory.max = 12,884,901,888 B (12 GiB)** from this dispatch's own cgroup. `test-safe-git-gc-limits` **33/33**; the remaining 15 battery suites plus `test-closed-bead-filter` (repo cwd) and `test-alert-triage-sweep` all pass. Live state: `.git` 106 MB / 127 loose / 840 KiB / 1 pack (12,607 objects / 100.70 MiB) / 0 garbage / 0 unpushed. Zero functional changes — every mitigation the crash motivates was already landed (report §9); this leg's only outputs are this row and the tasked report's Appendix B. Attribution notes: worktree `go build ./...` / `go test ./...` fail **only** inside a co-tenant's uncommitted `internal/watch/manager.go` edit (`domain.Parse` undefined; the two importers `internal/server` and `cmd/domain-check` fail on the same file) — every other package `ok`; the known untracked-only `resource_monitor*.go` gap since e4fcbec still stands. Box note: free disk read 19–20 GB at battery time (the [CRITICAL] threshold); three stale regenerable `~/scratch/*-target` dirs (≥20 h old, zero open handles, no cargo/rustc running) were cleared per the standing procedure → 22 GB — box state, not prevention. |
