...
### 5.5 Dispatch-named RCA record (domchk-e940d850, 2026-09-07 ~18:00 UTC)

The "Determine root cause and recommend mitigation for bf-1ea4g" dispatch names `docs/root-cause-analysis-bf-1ea4g-2026-08-13.md` as its output path — a path that had no file, so a **thin rendering** now exists there: it restates §1–§5's determination in this document's terms (no re-derivation, corrected-by-pointing only), walks the dispatch's four taxonomy branches (INFRASTRUCTURE = the kill, FALSE_POSITIVE = the alerts, SERVICE and CODE_DEFECT ruled out), and carries its own first-hand battery pass: gc-bound `--verify` exit 0 (≈3072 MiB), `test-gc-memory-bounds.sh` **16/16** with the death-operation push replay at peak RSS **231,468KB** (fifth independent pass in the 231–233 MB band), health exit 0 (104 MB, 88 loose / 668 KiB, 2 packs 99.78 MiB, 0 garbage, 0/0 divergence), hook `--check` exit 0, 0 tracked `.beads/` files, 8/8 timers, bundle manifest 6/6 (run from inside the bundle directory; its worktree README/MANIFEST carry a co-tenant's uncommitted +59-line addition, disclosed). Its census (same `^ALERT` filter as §5.4): **90** title-mentions — 69 closed / 15 open / 4 in_progress / 2 deferred — ALERT-shaped still pinned at **56** = one per kill; the dispatch bead itself is one of the 90 and resolves with that file as its named deliverable. Residuals restated with their tracking docs: H-1 (retry stop-condition) and M-2 (dispatch-scope telemetry), both NEEDLE-side.

### 5.6 Mitigation-implementation record (domchk-c68ae9bb, 2026-09-07 ~18:20 UTC)

The "Implement preventive measures based on bf-1ea4g root cause" dispatch names `docs/mitigation-implementation-bf-1ea4g-2026-08-13.md` as its output path — a path that had no file, so an **implementation record** now exists there: pointer-and-verification like §5.4, it walks the dispatch's INFRASTRUCTURE-branch task list (cleanup / gitignore / size documentation / monitoring) and re-proves each measure first-hand rather than landing anything new — its verdict, same as §5's: nothing new is owed on the mechanism. This dispatch's own battery: gc-bound `--verify` exit 0 (≈3072 MiB), `test-gc-memory-bounds.sh` **16/16** with the death-operation push replay at peak RSS **232,500KB** (sixth pass, same 231–233 MB band), health exit 0 (104 MB, 122 loose / 996 KiB, 2 packs 99.78 MiB, 0 garbage), hook `--check` exit 0, 0 tracked `.beads/` files, 8/8 timers, both live monitors PASS, alert suites green (`test-crash-alert-fixes.sh` exit 0, `test-closed-bead-filter.sh` 7/7), 0/0 divergence. **Two dispatch-premise corrections, dated here:** (1) "run `safe-git-gc.sh`" would be churn — the healthy-repo action is `--check-only`, whose exit 1 on "GC not needed" is the *verdict*; (2) "install `monitoring-setup.sh`" names the retired cron installer, which at HEAD is a silent no-op on this cron-less NixOS box (gap G-7) — the working entry point is `setup-repo-maintenance.sh` (8/8 timers verified); its co-tenant delegation rewrite is in flight uncommitted and was not committed here. **Scope signed, not done:** the FP branch's live CLASSIFICATION wiring defect (`crash-alert-manager.sh` taking the classifier banner as the classification at HEAD line 279 → dead FALSE_POSITIVE branch) is owned by open bead **domchk-f6fff20f** with its fix in flight in this worktree (robust token extraction at worktree lines 382–383); this dispatch touched neither script. H-1 and M-2 restated as NEEDLE-side residuals.
