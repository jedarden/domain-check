# Investigation Report (Draft): Domain-Check Crash Corpus

**Bead:** domchk-d6871df1 — step 2 of 4 (gather → **draft** → recommendations → commit)
**Chain:** domchk-65afcc88 (compiled findings, closed) → **domchk-d6871df1 (this draft)** → domchk-e843c4f1 (recommendations) → domchk-0d3c11c9 (commit)
**Umbrella:** domchk-11c26b24 "Document investigation results and recommendations" (parent bead bf-4829x8)
**Report date:** 2026-09-06 (all `[LIVE 09-06]` figures re-derived 2026-09-06 02:25 UTC)
...
operating limits (45 G mem available, 87 G disk free, load 0.41).
...
| `docs/investigations/evidence-compilation-2026-09-01-crash-investigation.md` | **untracked** | Sibling chain's artifact — the corpus's best provenance-tagged source. **Still uncommitted** (gap 2, §9). |

**What this report adds that no prior document contains in one place:** the corrected mechanism
(L1–L5), the verified timeline with per-claim provenance, the additions since the anchor report, and
an impact assessment that separates *verified* from *attested-only* figures.
...
`.git`, 1 pack / 90.34 MiB, 25 loose / 220 KiB, **0 garbage**.
...
pending** — the loop that produced this very chain is itself unremediated (§9, gap 5).
...
**Status: upstream fix needed.** No evidence of an upstream fix anywhere in the corpus.
...
- **A documentation-induced false alarm:** the documented gateway health check `curl -sf …` fails
  ...
  re-verified: `-sf` → exit 60; `-skf` → `ok`, exit 0. The repo's CLAUDE.md still showed the `-sf`
  form; corrected alongside this report.
...
crash-response effort belongs on infrastructure and workflow, not on this codebase.
...
| **2026-09-06** | **This draft.** All figures re-derived: repo 92 MB `.git`, 1 pack / 90.34 MiB, 25 loose / 220 KiB, 0 garbage; 15 memcg kills since Aug-17 all on Sep-02; last `exit_code=-1` 2026-08-17T16:00:26Z; host 45 G mem available / 87 G disk / load 0.41; gateway `-sf` exit 60 vs `-skf` `ok`. | [LIVE 09-06] |
...
6. **Three parallel synthesis chains were created** for the already-concluded investigation, including
   this one — the auto-split loop operating on a closed question (§9, gap 2/5).
...
| **Repository** | Healthy and optimal: 92 MB `.git`, 1 pack / 90.34 MiB, 25 loose / 220 KiB, 0 garbage; gc bounds holding | None — within the <500 MB healthy band | [LIVE 09-06] |
...
| **Host resources** | 45 G mem available / 62 G, 87 G disk free, load 0.41 — all far inside documented safe limits | None | [LIVE 09-06] |
...
| **Misdiagnosis risk** | The anchor report's opening still states the superseded mechanism; any reader starting there quotes "SIGHUP cascade / 94.71% oomd" | Medium — mitigated by the correction banner added to its header | [COMMIT] 383241f |
...
3. **bf-31mno — the largest single kill storm (434 kills) — has no dedicated RCA.** It is mentioned
   only inside bf-4yjq documents (§9, gap 4).
...
## 9. Open Gaps and Handoff to domchk-e843c4f1

Recommendations are step 3's deliverable; this section is the prioritized input, ordered by how much
each would reduce ongoing harm. Gaps 1 and 6 are **already actioned alongside this draft**.

| # | Gap | Suggested fix | Status |
|---|-----|---------------|--------|
| 1 | Anchor report leads with a superseded mechanism | Correction banner in its header | **Done with this draft** |
| 2 | Sibling chain stalled with its artifact uncommitted (domchk-59e1f1d5 open since 2026-09-01; three beads blocked) | Commit `evidence-compilation-2026-09-01-crash-investigation.md`, close 59e1f1d5, cascade-close | Open — fold into step 4 (0d3c11c9) |
| 3 | 23 investigation documents untracked, incl. the only copies of the systemic RCA and the exit-code−1 signal analysis | Housekeeping commit | Open — step 4 |
| 4 | bf-31mno (434 kills, largest single storm) has no dedicated RCA | One small RCA, or a cross-reference into the bf-173o7e record | Open |
| 5 | **Upstream NEEDLE fixes unverified** — no completion detection, no dedup, L3 release-conflict defect unfixed, auto-split loop unremediated | The substantive content for step 3 | **Primary recommendation target** |
| 6 | CLAUDE.md documents the false-alarming `curl -sf` gateway health check | One-line change to `-skf` | **Done with this draft** |
| 7 | Aug-12/13/14 claims permanently attested-only | Already handled — this report labels them explicitly | Closed by this draft |
| 8 | Single-workspace evidence scope | Cross-check fleet-wide figures against another workspace's needle log | Open — optional |
| 9 | No corpus index / superseded-pointer convention (the root cause of gap 1) | Adopt §7 as the canonical pointer list | Open — step 3 |

---

## 10. Live Re-Derivation (2026-09-06 02:25 UTC)

Commands behind every `[LIVE 09-06]` figure in this report:
...
*Draft — domchk-d6871df1. Baseline 383241f; input findings-compilation 5b4be43. Downstream:
domchk-e843c4f1 (recommendations, §9) → domchk-0d3c11c9 (commit, together with the sibling chain's
untracked artifact).*
